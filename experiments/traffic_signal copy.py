...
class TrafficSignal:
    ...
    def print_lane_info(self):
        """Prints information about lanes controlled by the traffic signal."""
        print(f"Lane information for Traffic Signal {self.id}:")
        for lane in self.lanes:
            length = self.sumo.lane.getLength(lane)
            vehicle_ids = self.sumo.lane.getLastStepVehicleIDs(lane)
            last_step_length = self.sumo.lane.getLastStepLength(lane)
            last_step_halting_number = self.sumo.lane.getLastStepHaltingNumber(lane)
        
            print(f"Lane ID: {lane}")
            print(f"Length: {length} meters")
            print(f"Vehicle IDs: {vehicle_ids}")
            print(f"Last Step Length: {last_step_length} meters")
            print(f"Last Step Halting Number: {last_step_halting_number}")
            print()
    ...
    def _priority_lane_reward(self):
        """Reward function that prioritizes lanes with density >= 80%."""
        high_density_threshold = 0.8  # 80% density
        for lane_density in self.get_lanes_density():
            if lane_density >= high_density_threshold:
                return 1.0  # High reward for high density lane
        return 0.0  # Low reward if no lane has high density
    ...
    def _vehicle_count_reward(self):
        """Reward function that gives higher reward for prioritizing lanes with more vehicles."""
        max_vehicles = 0
        for lane in self.lanes:
            vehicles = self.sumo.lane.getLastStepVehicleNumber(lane)
            max_vehicles = max(max_vehicles, vehicles)

        # Reward is proportional to the maximum vehicle count in any lane
        return max_vehicles
    ...
    reward_fns = {
        ...
        "priority-lane": _priority_lane_reward,  # Adding the new reward function
        "vehicle-count": _vehicle_count_reward,  # Adding the new reward function
    }
